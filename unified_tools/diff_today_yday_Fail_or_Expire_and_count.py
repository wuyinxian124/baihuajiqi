...
"""
脚本功能：
该脚本用于连接MySQL数据库，执行一个特定的SQL查询，该查询旨在获取昨天成功运行但今天运行失败或终止的详细信息。脚本将统计每个任务的记录数，并按租户ID和工作流ID进行排序。同时，脚本还会统计每个租户的任务数和实例数。
...
try:
    with connection_open.cursor() as cursor:
        ...
        WHERE
            ...
            AND (t.state IS NULL OR t.state = 3 OR t.state = 8) -- 今天的状态不是成功或没有状态
        ORDER BY
            y.tenant_id,
            y.task_type,
            y.project_id,
