...
def read_file(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if len(parts) == 4 and parts[1] == '[unified_scheduler]' and (parts[2] != 'Y' or parts[3] == 'O'):
...
def backup_and_delete_unified_task_run(task_ids, db_connection):
    ...
    try:
        ...
        SELECT * FROM lb_task_run WHERE task_id IN ({task_ids_str})
        """
        cursor.execute(backup_query, task_ids)

        # Delete records from lb_task_run
        delete_query = f"""
        DELETE FROM lb_task_run WHERE task_id IN ({task_ids_str})
        """
...
def main():
    if len(sys.argv) != 6:
        print("Usage: python script.py <file_path> <db_config_path> <ip> <port> ignoreIdStr")
        sys.exit(1)
    ...
    for j in range(0, len(valid_task_ids), 20):
        ...
        if response_data['data']['errorRows'] == 0 and not response_data['data']['failTask']:
            ...
            print("API call suc for return error ", response_data['data']['errorRows']," all ",response_data['data']['totalRows'])
